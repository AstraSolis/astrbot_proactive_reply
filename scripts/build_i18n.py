...
def _zh_pages() -> dict:
    return {
        ...
        "confirm_import_replace": "导入文件中含 {count} 条事项。选择「替换」将清空现有事项，选择「取消」可改用合并方式。是否替换？",
...
def _en_pages() -> dict:
    return {
        ...
        "confirm_import_replace": "The file has {count} event(s). Choosing Replace clears existing events; Cancel keeps them and merges. Replace?",
...
def _zh_api() -> dict:
    return {
        "errors": {
            ...
            "calendar_import_invalid": "导入数据不合法（应为事项数组）",
        },
        "messages": {
...
def _en_api() -> dict:
    return {
        "errors": {
            ...
            "calendar_import_invalid": "Invalid import data (expected an array of events)",
        },
        "messages": {
