...
class TestAIScheduleAnalyzer(unittest.TestCase):
    def test_contains_time_keywords(self):
        ...
        self.assertTrue(contains_time_keywords("有一点累，明早8点再聊"))
        self.assertTrue(contains_time_keywords("过一会回你"))
        self.assertTrue(contains_time_keywords("明天继续聊"))
        self.assertTrue(contains_time_keywords("晚上继续聊"))
        ...
        self.assertFalse(contains_time_keywords("再见"))  # 不应由"再"单字命中
        self.assertFalse(contains_time_keywords("早上好"))
        self.assertFalse(contains_time_keywords("晚上吃饭了吗"))
        self.assertFalse(contains_time_keywords("今天聊得很开心"))
        self.assertFalse(contains_time_keywords("到时候再说"))
        self.assertFalse(contains_time_keywords("改天再聊"))
        self.assertFalse(contains_time_keywords("我等了半小时"))
        self.assertFalse(contains_time_keywords("半小时后回来"))
        self.assertFalse(contains_time_keywords("明天继续努力"))
        self.assertFalse(contains_time_keywords("晚上继续努力"))

        # 陈述性表达不再为了召回而交给 LLM，避免日常消息制造隐性成本
        self.assertFalse(contains_time_keywords("这本书读了半天"))
