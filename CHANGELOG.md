...
- **插件市场展示优化**
  - 添加插件市场图标 (`logo.png`)
  - 在 `metadata.yaml` 中新增 `display_name` 字段，提升插件展示效果

- **主动消息分段发送功能**
  - 在 `send_proactive_message` 中实现消息分割逻辑
  - 新增 `split_message_delay_ms` 配置：控制分段发送的延迟时间（默认 500ms）
  - 新增 `split_by_backslash` 配置：控制是否启用分割功能（默认开启）
  - 历史记录保存完整原始消息，确保上下文连贯
  - 更新 `_conf_schema.json` 添加对应配置说明

- **消息分割功能支持正则表达式和多种预设模式**
  - 支持 `split_enabled` 和 `split_mode` 配置项控制消息分割行为
  - **6种预设分割模式**：
    - `backslash` - 按反斜线 (`\`) 分割
    - `newline` - 按换行符分割
    - `comma` - 按逗号分割
    ...
    - `punctuation` - 按标点符号分割
    - `custom` - 自定义正则表达式分割
  - 在 `custom` 模式下支持用户自定义正则表达式分割规则
  - Web 界面配置说明和正则表达式填写指南

### 改进

- **优化时间感知机制**
  - 在系统提示词中增加"时间信息使用指南"
  - 引导 LLM 使用模糊时间表述（如"刚才"、"最近"）替代具体时间点
  - 减少因 LLM 幻觉导致的时间描述错误
  - 提升主动消息的自然度和准确性
