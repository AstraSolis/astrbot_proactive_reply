...
- 支持自定义时间格式
- 支持自定义信息模板
- **智能注入**：动态信息和睡眠提示追加到用户消息后，固定指导词追加到 system_prompt 末尾，避免动态时间污染系统提示词缓存
...
- **睡眠时间设置**：睡眠时间内不发送主动消息，用户发消息时将睡眠提示追加到用户消息后
...
- **睡眠时间感知**：睡眠时间内用户发消息时，将睡眠提示追加到用户消息后
- **智能时间表达**：引导AI使用自然口语描述时间，避免机械式报时

### 智能时间表（日历事项）
- **日历样式日程表**：在 WebUI「时间表」页签以月历视图为任意日期添加「节日」或「事项」（如春节、生日、纪念日）
- **重复规则**：每条事项可设置不重复、重复 1–4 年（基准年 + 之后 N 年，共 N+1 年生效）或每年重复（永久）
...
- **AI 生成时间表**：在「时间表」页签输入主题或世界观（如「末世废土 / 幸存者据点的物资节、旧世界缅怀日」「蒸汽朋克都市 / 发明家庆典、齿轮节」），由 AI 一次性生成整套贴合主题的节日 / 纪念日；生成结果以可编辑预览呈现（每条可改月/日/名称/重复规则、逐条删除、手动新增一行），确认后可「追加到现有」或「清空并替换」；可在配置中指定生成所用的模型与系统提示词
...
- **持久化数据文件**：`data/plugin_data/astrbot_proactive_reply/persistent_data.yaml`
- **时间表数据文件**：`data/plugin_data/astrbot_proactive_reply/calendar_data.yaml`

**提示**：
- 持久化数据与时间表已采用 YAML 存储，更直观、便于查看；历史 `.json` 文件会在首次启动时自动迁移为 `.yaml`，旧文件备份为 `.json.bak`（如需回滚旧版本插件，可将其改回 `.json`）
...
### 用户信息附加设置
- **启用聊天附带用户信息**：启用后，在每次 LLM 请求前会自动将动态用户信息（用户名、时间等）追加到用户消息后；固定时间提示由对应配置追加到 system_prompt 末尾，睡眠提示仅在睡眠时追加到用户消息后
- **时间格式**：时间显示格式（Python datetime 格式）
- **信息模板**：用户信息的显示模板，支持占位符。会追加到用户消息后，不会覆盖人格设置
  - 支持的占位符：`{username}`, `{user_id}`, `{time}`, `{current_time}`, `{weekday}`, `{platform}`, `{chat_type}`, `{user_last_message_time}`, `{user_last_message_time_ago}`, `{ai_last_sent_time}`, `{calendar_today}`（今天的时间表事项，需启用「时间表」功能）
...
### 时间感知设置
- **时间感知增强提示词**：启用后，用户正常聊天时会将固定时间信息使用指南追加到 system_prompt 末尾；主动对话使用独立提示词构建
  - **自定义提示词**：可配置指导AI如何理解和表达时间的规则（模糊化表达、状态映射、间隔感知等）
- **睡眠时间功能**：
  - **启用开关**：启用后在睡眠时间段内不发送主动消息
  - **睡眠时间段**：在指定时间段内不发送消息，支持跨午夜（如 22:00-8:00）
  - **睡眠提示内容**：在睡眠时间段内追加到用户消息后，让AI感知当前处于睡眠时间
...
- **分割模式**：选择消息分割的方式,提供8种模式
  - 预设模式：backslash、newline、comma、semicolon、punctuation、custom
  - 官方风格模式：words（分段词列表）、regex（正则表达式）
- **自定义分割正则表达式**：仅当分割模式为custom时使用
- **正则表达式（regex模式）**：仅当分割模式为regex时使用，使用官方风格的正则表达式
- **分段词列表（words模式）**：仅当分割模式为words时使用，列表中的词会被用作分段标记
- **分割消息延迟**：分割后的消息之间的发送延迟,单位毫秒(默认500ms)

### AI 自主调度设置

- **启用 AI 自主调度**：启用后，AI 在对话中提到时间约定（如"40分钟后找你"）时，插件会自动分析并设置一次性定时任务
  - 分析会消耗一次额外的 LLM 调用
  - 使用关键词预检测，只有包含时间相关关键词的消息才会触发分析
- **AI 调度分析使用的模型提供商**：指定用于 AI 调度分析的模型提供商
  - 留空则使用主模型（与用户对话相同的模型）
  - 可以选择不同的模型以使用更便宜或更快的模型进行调度分析，节省成本
- **AI 调度分析提示词**：自定义 AI 调度分析的系统提示词
  - 用于指导 AI 判断消息中是否包含时间约定并提取信息
  - 支持自动注入已有待执行的约定，避免重复创建相同任务

### AI 生成时间表设置

- **AI 生成时间表使用的模型提供商**：指定用于「AI 生成时间表」的模型提供商
  - 留空则使用当前正在使用的主模型
  - 也可在 WebUI「时间表」页签的「AI 生成」面板临时切换模型
- **AI 生成时间表的系统提示词**：自定义生成时间表的系统提示词
  - 指导 AI 按 JSON 数组格式输出贴合主题的日历事项（`month` / `day` / `text` / `repeat`）
  - 插件会在该提示词后自动补充当前年份与数量上限等运行时约束
  - 使用方式：在「时间表」页签输入主题（如「末世废土 / 幸存者据点的物资节、旧世界缅怀日」），点击「生成」后在可编辑预览中逐条调整 / 删除 / 新增，再选择「追加到现有」或「清空并替换」
...
**重要提示**：
- 动态用户信息和睡眠提示会追加到用户消息后；时间感知等固定指导词会追加到 system_prompt 末尾
...
- **人格协同机制**：AstrBot人格保留在 system_prompt，主动对话指令作为本轮用户提示词传递
...
2. **独立持久化文件**：保存到 `data/plugin_data/astrbot_proactive_reply/persistent_data.yaml`，避免配置重置影响
...
**提示**：
1. 请根据 [AstrBot 官方文档](https://docs.astrbot.app/) 正确配置对应的平台适配器
2. 每个平台的会话是独立的，需要**在对应平台上执行命令**才能添加该平台的会话
