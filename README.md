...
- 支持自定义时间格式
- 支持自定义信息模板
- **智能注入**：不会覆盖现有人格设置，动态信息追加到用户消息后，保持 system_prompt 稳定
...
### 用户信息附加设置
- **启用聊天附带用户信息**：启用后，在每次 LLM 请求前会自动将用户信息（用户名、时间等）追加到用户消息后，不修改人格 system_prompt
...
### 时间感知设置
- **时间感知增强提示词**：启用后，用户正常聊天时会在用户消息后追加时间信息使用指南；主动对话使用独立提示词构建
...
**重要提示**：
- 用户信息会追加到用户消息后，不会修改人格 system_prompt
...
- **用户信息注入**：通过 extra_user_content_parts 追加到用户消息后，保持 system_prompt 稳定、人格完整
